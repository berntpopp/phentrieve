...
def main():
    ...
    try:
        ...
        try:
            ...
            logging.info(
                f"Connected to ChromaDB collection '{collection_name}' with {collection.count()} entries."
            )
        except Exception as e:
            ...
            try:
                ...
                logging.info(
                    f"Connected to ChromaDB collection 'hpo_multilingual' with {collection.count()} entries."
                )
            except Exception as e:
                logging.error(
                    f"Error: No suitable collection found for model '{args.model_name}'"
                )
                logging.error(
                    f"Make sure you've run setup_hpo_index.py with this model or the default model"
                )
                sys.exit(1)
    except Exception as e:
