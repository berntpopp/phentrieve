...
docker build \
  --build-arg BUNDLE_URL="https://github.com/berntpopp/phentrieve-data/releases/download/hpo-v2026-06-23-r2/phentrieve-data-v2026-06-23-biolord-multivec.tar.gz" \
...
**Why read-write for data directory?**
- `hpo_data.db`: HPO SQLite database generated by `phentrieve data prepare`
- `hp.json`: Downloaded HPO ontology file
...
2. **Missing HPO data** (most common for fresh deployments):
   ```bash
   ...
   **Error message**: `HPO database not found: .../hpo_data.db`

   This means the SQLite database hasn't been created. The `data prepare` command downloads the HPO ontology and generates the required database file.
