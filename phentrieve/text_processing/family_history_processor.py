...
FAMILY_HISTORY_REGEX = re.compile(
    '|'.join(FAMILY_HISTORY_PATTERNS),
    re.IGNORECASE
)
...
def _is_valid_phenotype_text(text: str) -> bool:
    ...
    stopwords = {
        'the', 'and', 'or', 'but', 'with', 'for', 'is', 'was', 'has', 'had',
        'been', 'being', 'have', 'are', 'were', 'will', 'would', 'should',
        'could', 'may', 'might', 'can', 'must', 'shall', 'significant',
        'history', 'family', 'member', 'relative', 'maternal', 'paternal',
    }
...
def process_family_history_chunks(
    ...
    for chunk_result in chunk_results:
        ...
        if is_family_history_chunk(chunk_text):
            logger.debug(f"Detected family history in chunk {chunk_idx}: {chunk_text[:50]}...")
            ...
            if phenotype_extractions:
                ...
                for ext, query_result in zip(phenotype_extractions, batch_results):
                    if not query_result.get("metadatas") or not query_result["metadatas"][0]:
                        logger.debug(f"No query results for phenotype '{ext.phenotype_text}'")
                        continue
                    ...
                    for i, metadata in enumerate(query_result["metadatas"][0]):
                        similarity = query_result.get("distances", [[]])[0][i] if query_result.get("distances") else 0.0
                    ...
                    if phenotype_matches:
                        ...
                        synthetic_chunk = {
                            ...
                            "start_char": chunk_result.get("start_char", 0) + ext.start_char,
                            "end_char": chunk_result.get("start_char", 0) + ext.end_char,
