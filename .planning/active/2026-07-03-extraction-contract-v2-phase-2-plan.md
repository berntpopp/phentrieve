...
>
> **Rev 2** -- revised after a self-review + Codex review against the real code. Changes from rev 1: fixed a Task-2 validation bug, corrected the assertion-vocabulary strategy (three vocabularies, not two), added the missing axis-threading sites in `_retrieve_candidates`, moved the B3 emit point to the correct place in the resolve flow, fixed the `capabilities_version` and MCP-projection mechanisms, and corrected the frontend test path + prompt glob. See "Review deltas" at the end.
...
- **D3 -- compat `category`.** Where category-dependent logic remains (actionability `pipeline.py:340-344`; Phase-1 dedup key `pipeline_phase1.py:209-217`; mapping payload `pipeline_phase2.py:102-110`), derive a compat `category` from the canonical `experiencer`+`assertion` so the model's axes win when they disagree with the legacy enum, while existing category consumers keep working.
...
- [ ] **Step 2: Run it and confirm it fails** -- `cd frontend && npm run test -- usePhenotypeCollection` -> FAIL for `'absent'`.
...
- [ ] **Step 4: Run test + gate** -- `cd frontend && npm run test -- usePhenotypeCollection` then `make frontend-test-ci` -> passed.
...
**Interfaces:** Consumes the candidate dict with `"assertion"`/`"experiencer"` (Task 4); `parse_assertion` (`llm/utils.py:76-88`, wire->pipeline vocab). Produces: `phenotype_from_candidate` sets `assertion = parse_assertion(item["assertion"]).value` (pipeline vocab `present/negated/uncertain`) when the model supplied it, else the category fallback; `experiencer` from the model with `experiencer_for_category` fallback; a compat `category` derived from the axes (D3). Dedup key `(term_id, experiencer, assertion)` unchanged (`:342`).
...
- [ ] **Step 1: Write the failing test** -- create `tests/unit/llm/test_phase2_assertion_source.py`: an `item` whose `category` maps to `present` via `CATEGORY_TO_ASSERTION` but whose model `assertion="absent"` -> resulting phenotype `assertion == "negated"` (model wins). Second case: no `assertion` key -> category-derived fallback (backward compatible).
...
Add `from phentrieve.llm.utils import parse_assertion`. Keep/derive `category` as a compat field. Confirm the dedup key at `:342` is unchanged.
...
**Files:** Modify `phentrieve/text_processing/full_text_service.py` (`_adapt_llm_aggregated_terms` :434-455 -- carry `experiencer`; `adapt_shared_service_response_to_api` -- surface the family list); `api/schemas/text_processing_schemas.py` (`AggregatedHPOTermAPI` :189-225 add `experiencer`; `TextProcessingResponseAPI` :228-263 add `family_history_findings`); `api/services/text_processing_execution.py` (builder :141-166 -- copy both); `api/mcp/schemas.py` (`EXTRACT_SCHEMA` :43-47 add `family_history_findings`); `api/mcp/capabilities.py` (descriptor :107-118 -- roll `capabilities_version`); `api/mcp/projection.py` (:69 `dict(term)` copies all keys -- verify `experiencer` survives; project the family list); `api/mcp/shaping.py` + `tools/retrieval.py` (:171-172,:248-249 -- a second `enforce_budget(list_field="family_history_findings")`). Test `tests/unit/api/test_family_findings_surface.py`, `tests/unit/mcp_server/test_mcp_family_projection.py`.

**Interfaces:** `family_history_findings` on REST + MCP extract envelope; `experiencer` preserved per term; the family list budgeted independently.

> **Fixes vs rev 1:** (a) the term `experiencer` is dropped at `_adapt_llm_aggregated_terms` (`full_text_service.py:434-455`), NOT at MCP projection (`projection.py:69` does `dict(term)` and its drop list `:35-47` excludes `experiencer`); fix the earlier layer. (b) `capabilities_version` hashes `capabilities.py`'s descriptor (`:207-223`, `:107-118`), NOT `EXTRACT_SCHEMA`; editing `schemas.py` alone will not roll it -- update the descriptor / bump `_api_version`.

- [ ] **Step 1: Write the failing tests** -- `test_family_findings_surface.py`: `TextProcessingResponseAPI` round-trips a `family_history_findings` entry + per-term `experiencer`. `test_mcp_family_projection.py`: the MCP extract envelope includes `family_history_findings`, each term keeps `experiencer`, and `capabilities_version()` differs from its pre-change value (roll asserted).
...
**Interfaces:** Produces, when `Y` maps above the floor, a generated finding: `hpo_id`, `label`, `assertion="negated"` (pipeline vocab), `qualifier_surface_text="Y"`, evidence/attribution, `match_method="negated_qualifier_derived"`, `confidence >= floor`. Below floor -> keep today's metadata string, no generated term.
...
- [ ] **Step 1: Write the failing test** -- a `negated_qualifier_derived` finding round-trips through `AggregatedHPOTermAPI` with the fields populated; the response builder copies them.
